...
(defun ->cnf (p &optional vars)
    ...
    (case (op p)
      ...
      (OR  (merge-disjuncts (mapcar #'(lambda (q) (->cnf q vars))
                                    (args p))))
      (FORALL (let ((new-vars (mapcar #'new-variable  (ensure-list (arg1 p)))))
                  (->cnf (sublis (mapcar #'cons  (ensure-list (arg1 p)) new-vars)
                                 (arg2 p))
...
(defun ->inf- (p)
    "Convert a sentence p to implicative normal form [p 282]."
    (let ((inf))
        (dolist (it (conjuncts (->cnf p)) (conjunction (reverse inf)))
            (push (cnf1->inf1 it) inf))))
...
(defun renaming? (p q &optional (bindings +no-bindings+))
  ...
  only in variable names?)"
    ;;(print (list 'renaming p q bindings))
...
(defun atomic-clause? (sentence)
    "An atomic clause has no connectives or quantifiers."
    (not (or (member (intern (symbol-name (op sentence)) :horn) +logical-connectives+)
             (member (intern (symbol-name (op sentence)) :horn) +logical-quantifiers+))))
|#
